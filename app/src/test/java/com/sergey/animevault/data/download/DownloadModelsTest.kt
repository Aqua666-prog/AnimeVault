...
class DownloadModelsTest {
    ...
    @Test
    fun chooseDownloadStream_canRequireExactQuality() {
        val streams = listOf(
            stream("720", 720, OnlineStreamType.HLS),
            stream("1080", 1080, OnlineStreamType.HLS),
        )

        assertThat(chooseDownloadStream(streams, null, 720, allowQualityFallback = false)?.id)
            .isEqualTo("720")
    }

    @Test
    fun chooseDownloadStream_strictQualityReturnsNullWhenUnavailable() {
        val streams = listOf(stream("720", 720, OnlineStreamType.HLS))

        assertThat(chooseDownloadStream(streams, null, 1080, allowQualityFallback = false))
            .isNull()
    }


    @Test
    fun chooseDownloadStream_strictQualityAllowsUnknownMasterHls() {
        val master = stream("master", null, OnlineStreamType.HLS)

        assertThat(chooseDownloadStream(listOf(master), null, 1080, allowQualityFallback = false)?.id)
            .isEqualTo("master")
    }
