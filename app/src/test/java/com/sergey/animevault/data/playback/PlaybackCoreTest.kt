...
class PlaybackCoreTest {
    ...
    @Test
    fun nativeHttpIOException_isClassifiedByStatusCode() {
        assertThat(PlaybackFailureClassifier.classify(java.io.IOException("HTTP 403 при загрузке")).kind)
            .isEqualTo(PlaybackFailureKind.FORBIDDEN)
        assertThat(PlaybackFailureClassifier.classify(java.io.IOException("HTTP 416 при возобновлении")).kind)
            .isEqualTo(PlaybackFailureKind.NOT_FOUND)
    }
}
