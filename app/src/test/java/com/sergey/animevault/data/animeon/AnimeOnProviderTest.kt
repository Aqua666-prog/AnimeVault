...
class AnimeOnProviderTest {
    ...
    @Test
    fun `episode zero is preserved for specials`() {
        val translation = AnimeOnTranslationOption(
            translationId = 1,
            playerId = 2,
            name = "Voice",
        ...
        val episodes = mergeAnimeOnEpisodeSets(
            ...
            episodeSets = listOf(
                translation to listOf(
                    AnimeOnEpisodeDto(episode = 0, fileUrl = "https://cdn.example/special.m3u8"),
                    AnimeOnEpisodeDto(episode = 1, fileUrl = "https://cdn.example/episode1.m3u8"),
                ),
            ),
        )

        assertThat(episodes.mapNotNull { it.ordinal }).containsExactly(0.0, 1.0).inOrder()
        assertThat(episodes.first().name).isEqualTo("0 серия")
    }
