...
class ProviderHealthTrackerTest {
    ...
    @Test
    fun downloadFailuresDoNotPoisonCatalogChannel() {
        val tracker = ProviderHealthTracker()
        tracker.recordSuccess("demo", ProviderOperation.CATALOG, 100L)
        repeat(3) {
            tracker.recordFailure(
                ...
                latencyMs = 1_000L,
                error = SocketTimeoutException("download CDN slow"),
                sourceName = "Demo",
            )
        }

        val state = tracker.states.value.getValue("demo")
        assertEquals(ProviderHealthStatus.AVAILABLE, state.channel(ProviderHealthChannel.CATALOG).status)
        assertEquals(ProviderHealthStatus.UNAVAILABLE, state.channel(ProviderHealthChannel.DOWNLOAD).status)
        assertTrue(tracker.shouldAttempt("demo", ProviderHealthChannel.CATALOG))
