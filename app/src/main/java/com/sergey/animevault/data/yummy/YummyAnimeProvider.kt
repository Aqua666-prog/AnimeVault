 ...
 *
 * Yani requires a public application key in the X-Application header. AnimeVault keeps
 * that application key in Android Keystore and never confuses it with a user's Bearer token.
 *
...
class YummyAnimeProvider internal constructor(
    ...
    override val descriptor = OnlineProviderDescriptor(
        ...
        name = "YummyAnime",
        description = "Каталог YummyAnime через Yani API; требуется X-Application ключ приложения",
        authMode = ProviderAuthMode.REQUIRED_TOKEN,
        isExperimental = true,
    ...
    private suspend fun <T> requestJson(url: String, type: Class<T>): T {
        val token = token()?.takeIf(String::isNotBlank)
            ?: throw OnlineSourceException("YummyAnime: укажите X-Application ключ в настройках источника")
    ...
    private companion object {
        ...
        const val SITE_BASE = "https://yummyani.me/"
...
internal fun mergeYummyEpisodes(animeId: Int, videos: List<YummyVideoDto>): List<OnlineEpisode> {
    ...
    return groups.entries.mapIndexed { groupIndex, (number, variants) ->
        ...
        val streams = variants.mapNotNull { indexed ->
            ...
            val url = rawUrl.absoluteUrl("https://yummyani.me")
            OnlineStream(
                ...
                headers = mapOf("Referer" to "https://yummyani.me/"),
...
private fun YummyPosterDto?.bestUrl(): String? = listOfNotNull(
    ...
    this?.medium,
    this?.small,
).firstOrNull { it.isNotBlank() }?.absoluteUrl("https://yummyani.me")
