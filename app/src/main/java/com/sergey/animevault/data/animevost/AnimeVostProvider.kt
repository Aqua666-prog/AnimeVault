...
class AnimeVostProvider internal constructor(
    ...
    override suspend fun getRelease(id: String): OnlineReleaseDetails {
        val numericId = id.toIntOrNull()?.takeIf { it > 0 }
            ?: throw OnlineSourceException("AnimeVost: некорректный ID релиза")
        val item = releaseCache[id]
            ?: runCatching { api.search(id) }.getOrNull()
                ?.data
                ?.firstOrNull { it.id == numericId }
            ?: runCatching { api.getLatest(page = 1, quantity = 100) }.getOrNull()
                ?.data
                ?.firstOrNull { it.id == numericId }
            // Playlist lookup is authoritative by numeric id. Metadata may have fallen out of the
            // latest feed, so keep old/history entries playable instead of pretending they vanished.
            ?: AnimeVostItemDto(id = numericId, title = "AnimeVost #$numericId")
        releaseCache[id] = item
        val playlist = api.getPlaylist(numericId)
