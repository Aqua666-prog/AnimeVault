...
abstract class AnimeVaultDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        val MIGRATION_7_8 = object : Migration(7, 8) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE `downloads` ADD COLUMN `stream_provider_id` TEXT")
                db.execSQL("ALTER TABLE `downloads` ADD COLUMN `stream_provider_name` TEXT")
                db.execSQL("ALTER TABLE `downloads` ADD COLUMN `source_host` TEXT")
                db.execSQL("ALTER TABLE `downloads` ADD COLUMN `speed_bytes_per_second` INTEGER NOT NULL DEFAULT 0")
                db.execSQL("ALTER TABLE `downloads` ADD COLUMN `eta_seconds` INTEGER")
                db.execSQL("ALTER TABLE `downloads` ADD COLUMN `attempt_count` INTEGER NOT NULL DEFAULT 0")
                db.execSQL("ALTER TABLE `downloads` ADD COLUMN `failure_kind` TEXT")
            }
        }
