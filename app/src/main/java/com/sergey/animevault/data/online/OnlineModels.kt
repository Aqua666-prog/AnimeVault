    ...
    companion object {
        fun aggregate(items: Collection<ProviderCapabilities>): ProviderCapabilities {
            if (items.isEmpty()) return ProviderCapabilities(
                ...
                streams = false, translations = false, subtitles = false, directPlayback = false, downloads = false,
            ...
            return ProviderCapabilities(
                ...
                downloads = items.any { it.downloads },
...
enum class ProviderHealthChannel {
    CATALOG,
    PLAYBACK,
    DOWNLOAD,
}

data class ProviderChannelHealthState(
...
data class ProviderHealthState(
    ...
    val lastFailureKind: PlaybackFailureKind? = null,
    val cooldownUntilMs: Long? = null,
    val channels: Map<ProviderHealthChannel, ProviderChannelHealthState> = emptyMap(),
) {
    fun channel(channel: ProviderHealthChannel): ProviderChannelHealthState =
        channels[channel] ?: ProviderChannelHealthState()
}
